...
const ArticleDisplay: React.FC<ArticleDisplayProps> = ({ 
  ...
  if (chunks.length === 0) {
    return (
      <div className="glass-panel rounded-[24px] p-8 sm:p-10 shadow-luxe">
        <div className="flex flex-col items-center justify-center h-72 text-ink/50 border border-dashed border-ink/15 rounded-[20px] bg-paper">
  ...
  return (
    <div className="glass-panel rounded-[24px] p-8 sm:p-12 shadow-luxe mb-32 relative">
      <div className="absolute top-0 left-0 right-0 h-px bg-ink/10"></div>

      <div className="space-y-8 font-sans text-xl sm:text-2xl leading-loose text-ink/70">
            ...
            <div 
              ...
              className={`
                ...
                ${isActive 
                  ...
                  : 'bg-white border-transparent text-ink/60 hover:border-ink/15'
                }
              `}
            >
              {/* Active Marker */}
              {isActive && (
                <div className="absolute left-0 top-6 bottom-6 w-1 bg-accent rounded-r-full shadow-[0_0_12px_rgba(0,102,255,0.45)]"></div>
              )}
